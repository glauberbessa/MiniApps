...
export async function POST(request: NextRequest) {
  ...
  try {
    logger.info("API", "POST /api/playlists/transfer - Request started", { traceId });

    const session = await auth();

    logger.info("API", "POST /api/playlists/transfer - Session retrieved", {
      traceId,
      hasSession: !!session,
    ...
    if (!session?.user?.id || !session.accessToken) {
      ...
      logger.warn("API", "POST /api/playlists/transfer - Unauthorized", { traceId, reason });
      clearTraceId();
    ...
    logger.info("API", "POST /api/playlists/transfer - Request body parsed", {
      traceId,
    ...
    if (!body.sourcePlaylistId || !body.destinationPlaylistId) {
      logger.warn("API", "POST /api/playlists/transfer - Missing playlist IDs", { traceId });
      clearTraceId();
      return NextResponse.json({ error: "IDs das playlists são obrigatórios", traceId }, { status: 400 });
    }

    if (!body.videos || body.videos.length === 0) {
      logger.warn("API", "POST /api/playlists/transfer - No videos selected", { traceId });
      clearTraceId();
      return NextResponse.json({ error: "Nenhum vídeo selecionado", traceId }, { status: 400 });
    }

    if (body.sourcePlaylistId === body.destinationPlaylistId) {
      logger.warn("API", "POST /api/playlists/transfer - Same source and destination", { traceId });
      clearTraceId();
    ...
    logger.info("API", "POST /api/playlists/transfer - Quota check", {
      traceId,
      requiredQuota,
      hasQuota,
    ...
    if (!hasQuota) {
      logger.warn("API", "POST /api/playlists/transfer - Insufficient quota", { traceId, requiredQuota });
      clearTraceId();
    ...
    logger.info("API", "POST /api/playlists/transfer - Request completed", {
      traceId,
    ...
    const totalElapsed = Date.now() - startTime;
    logger.error("API", "POST /api/playlists/transfer - Request FAILED", error instanceof Error ? error : undefined, {
      traceId,
