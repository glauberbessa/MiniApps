import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Youtube, ListVideo, Radio, Gauge } from "lucide-react";
import { UI_TEXT } from "@/lib/i18n";
import { signInWithGoogle } from "./actions";

export default function LoginPage() {
  return (
    <div className="w-full max-w-md">
      <div className="border border-neutral-800 bg-neutral-950 p-8">
        <div className="text-center mb-8">
          <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center bg-white">
            <Youtube className="h-10 w-10 text-black" />
          </div>
          <h1 className="text-3xl font-bold tracking-tighter mb-2">{UI_TEXT.auth.loginTitle}</h1>
          <p className="text-neutral-400">{UI_TEXT.auth.loginSubtitle}</p>
        </div>
        
        <div className="space-y-8 mb-8">
          {/* Features */}
          <div className="space-y-6">
            <Feature
              icon={ListVideo}
              title="Gerenciar Playlists"
              description="Organize e transfira vídeos entre playlists"
            />
            <Feature
              icon={Radio}
              title="Gerenciar Canais"
              description="Atribua vídeos de canais às suas playlists"
            />
            <Feature
              icon={Gauge}
              title="Monitorar Quota"
              description="Acompanhe seu consumo da API do YouTube"
            />
          </div>

          {/* Login Button - Using server action to fix PKCE cookie issues */}
          <form action={signInWithGoogle}>
            <Button
              type="submit"
              className="w-full rounded-none bg-white text-black hover:bg-neutral-200 h-12 font-bold tracking-tight"
              size="lg"
            >
              <svg
                className="mr-2 h-5 w-5"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fill="currentColor"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="currentColor"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="currentColor"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="currentColor"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              {UI_TEXT.auth.loginWithGoogle}
            </Button>
          </form>

          <p className="text-center text-xs text-neutral-500">
            {UI_TEXT.auth.loginDescription}
          </p>
        </div>
      </div>
    </div>
  );
}

function Feature({
  icon: Icon,
  title,
  description,
}: {
  icon: React.ElementType;
  title: string;
  description: string;
}) {
  return (
    <div className="flex items-start gap-4 group">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center bg-neutral-900 border border-neutral-800 group-hover:border-neutral-700 transition-colors">
        <Icon className="h-5 w-5 text-white" />
      </div>
      <div>
        <p className="text-sm font-bold text-white">{title}</p>
        <p className="text-xs text-neutral-400">{description}</p>
      </div>
    </div>
  );
}
