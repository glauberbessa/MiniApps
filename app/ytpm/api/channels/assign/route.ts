...
export async function POST(request: NextRequest) {
  ...
  try {
    logger.info("API", "POST /ytpm/api/channels/assign - Request started", { traceId });

    const session = await auth();

    logger.info("API", "POST /ytpm/api/channels/assign - Session retrieved", {
      traceId,
      hasSession: !!session,
    ...
    if (!session?.user?.id || !session.accessToken) {
      ...
      logger.warn("API", "POST /ytpm/api/channels/assign - Unauthorized", { traceId, reason });
      clearTraceId();
    ...
    logger.info("API", "POST /ytpm/api/channels/assign - Body parsed", {
      traceId,
    ...
    logger.info("API", "POST /ytpm/api/channels/assign - Quota check", {
      traceId,
      requiredQuota,
      hasQuota,
    });
    ...
    logger.info("API", "POST /ytpm/api/channels/assign - Request completed", {
      traceId,
    ...
    const totalElapsed = Date.now() - startTime;
    logger.error("API", "POST /ytpm/api/channels/assign - Request FAILED", error instanceof Error ? error : undefined, {
      traceId,
